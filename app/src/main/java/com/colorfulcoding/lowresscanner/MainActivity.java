...
public class MainActivity extends AppCompatActivity {
        ...
        fragment.getArSceneView().getScene().addOnUpdateListener(frameTime -> {
            ...
            try {
                ...
                for(int i = 0; i < points.limit(); i += 4) {
                    ...
                    if(Math.abs(points.get(i)) > 1 || Math.abs(points.get(i+1)) > 1 || Math.abs(points.get(i+2)) > 1)  continue;
                    ...
                    if(color == null || color.length != 3) color = new int[]{0,0,0};
    ...
    private void createJsonFromFeaturePoints(View v){
        ...
        File truu = new File(file, "test.xyz");
        try {
            FileWriter writer = new FileWriter(truu);
            for(int i=0;i<positions3D.size();i++){
                float x = positions3D.get(i)[0];
                float y = positions3D.get(i)[1];
                float z = positions3D.get(i)[2];
                int r = colorsRGB.get(i)[0];
                int g = colorsRGB.get(i)[1];
                int b = colorsRGB.get(i)[2];
